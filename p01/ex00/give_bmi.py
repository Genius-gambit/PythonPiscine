...
def	give_bmi(height: list[int | float], weight: list[int | float])\
	...
	try:
		...
		for i in range(len(height)):
			if (not isinstance(height[i], int) and not isinstance(height[i], float) or\
				not isinstance(weight[i], int) and not isinstance(weight[i], float)):
				raise AssertionError("The type of element of list is not valid")
...
def apply_limit(bmi: list[int | float], limit: int) -> list[bool]:
	...
	Returns:
		Returns the list of boolean elements if element is greater than the limit.
	"""
